...
interface Props {
  ...
  unit: Unit;
  onClose: () => void;
  onSave: (updatedUnit: Unit, photo: File | null) => void;
}

const EditUnitModal: React.FC<Props> = ({ isOpen, unit, onClose, onSave }) => {
  const [unitName, setUnitName] = useState(unit.name);
  const [photoUrl, setPhotoUrl] = useState(unit.photo);
  const [compressedFile, setCompressedFile] = useState<File | null>(null); 

  const handleSave = () => {
      const updatedUnit = { ...unit, name: unitName} ;
      onSave(updatedUnit, compressedFile);
      onClose();  
  };
