...
export default function UserMetaCard() {
    ...
    const [userData, setUserData] = useState({
        ...
        facebook: user?.user.user.facebook || "",
        instagram: user?.user.user.instagram || "",
        linkedin: user?.user.user.linkedin || "",
        youtube: user?.user.user.youtube || "",
        biografia: user?.user.user.biografia || "",
    ...
    useEffect(() => {
        ...
        if (user?.user.user) {
            ...
            setPreviewImage(user.user.user.photoUrl || "");
        }
    }, [user]);
    ...
    const handleSave = async () => {
        try {
            ...
            const payload = {
                ...
                photoUrl: uploadedImageUrl || userData.photoUrl,
            ...
            await userService.updateUser(user.user.user.id, payload);
    ...
    return (
        <>
            <div className="p-6 border border-gray-200 rounded-2xl shadow-sm dark:border-gray-800 lg:p-6 transition-all duration-300 hover:shadow-md">
                <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                    <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
                        <div className="w-24 h-24 overflow-hidden rounded-full shadow-md hover:shadow-primary/10 transition-all duration-300">
            ...
            {/* EDITAR INFORMAÇÕES PESSOAIS - Enhanced Modal */}
            <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
                <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-8">
                    ...
                    <div className="custom-scrollbar max-h-[70vh] overflow-y-auto px-2 pb-3">
                        <div className="mb-8">
                            <h5 className="mb-4 text-lg font-medium text-gray-800 dark:text-white/90 flex items-center">
                                <User2 size={18} className="mr-2 text-primary" />
                            ...
                            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                                    ...
                                    <div className="flex items-center space-x-4 mt-2">
                                        {previewImage ? (
                                            <img
                                                src={previewImage}
                                                alt="Preview"
                                                className="h-16 w-16 object-cover rounded-full border-2 border-primary/20"
                                            />
                                        ) : (
                                            <div className="h-16 w-16 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                                                <User2 className="text-gray-500 dark:text-gray-400" size={24} />
                                            </div>
                                        )}
                                        <label className="cursor-pointer">
                                            <span className="inline-flex items-center px-4 py-2 rounded-full border border-gray-300 dark:border-gray-600 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-300">
                                                Alterar imagem
                                            </span>
                                            <input
                                                type="file"
                                ...
                                <div className="lg:col-span-2">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full px-4 py-2 mt-1 border border-gray-300 rounded-lg dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-primary/50 focus:border-primary"
                                        rows={3}
                                    />
                                </div>
                            </div>
                        ...
                        <div className="border-t dark:border-gray-700 pt-6 mb-6">
                            ...
                            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                                ...
                                <div>
                                    <Label>
                                        <span className="flex items-center">
                                            <svg className="w-4 h-4 mr-2 text-blue-700" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <div>
                                    <Label>
                                        <span className="flex items-center">
                                            <svg className="w-4 h-4 mr-2 text-pink-600" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <div>
                                    <Label>
                                        <span className="flex items-center">
                                            <svg className="w-4 h-4 mr-2 text-red-600" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div className="mt-6 flex items-center justify-end gap-3 border-t pt-4 dark:border-gray-700">
                        <Button
                            onClick={closeModal}
                            variant="outline"
                            className="gap-2 border-gray-300 text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-800"
                        >
                            <X size={16} />
                            Cancelar
                        </Button>
...
//   const [facebook, setFacebook] = useState(user?.user.user.facebook ?? "");
//   const [instagram, setInstagram] = useState(user?.user.user.instagram ?? "");
//   const [linkedin, setLinkedin] = useState(user?.user.user.linkedin ?? "");
//   const [youtube, setYoutube] = useState(user?.user.user.youtube?? "");
//   const [biografia, setBiografia] = useState(user?.user.user.biografia ?? "");

//   const [imageFile, setImageFile] = useState<File | null>(null);
//   const [previewImage, setPreviewImage] = useState(user?.user.user.photoUrl ?? "");
...
//     setFacebook(user?.user.user.facebook ?? "");
//     setInstagram(user?.user.user.instagram ?? "");
//     setLinkedin(user?.user.user.linkedin ?? "");
//     setYoutube(user?.user.user.youtube ?? "");
//     setBiografia(user?.user.user.biografia ?? "");
//     setImageFile(null);
//     setPreviewImage(user?.user.user.photoUrl ?? "");
//   }
...
//       await userService.updateUser(user.user.user.id, payload);
//         toast.success("Dados atualizados com sucesso", { position: 'bottom-right' });
//         resetForm();
//         closeModal();
//         setIsLoading(false);
...
//                 <p className="text-sm text-gray-500 dark:text-gray-400">
//                   {user?.user.user.biografia ? user?.user.user.biografia : user?.user.user.email}
//                 </p>
//               </div>
//             </div>
...
//               {
//                 user?.user.user.facebook && (
//                     <a
//                       href={user?.user.user.facebook}
//                       target="_blank"
//                       rel="noopener"
...
//               }
//               {
//                 user?.user.user.youtube && (
//                       <a
//                         href={user?.user.user.youtube}
//                         target="_blank"
//                         rel="noopener"
//                         className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"
//                       >
...
//               {
//                 user?.user.user.linkedin && (
//                    <a
//                       href={user?.user.user.linkedin}
//                       target="_blank"
//                       rel="noopener"
//                       className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"
//                     >
...
//               }
//               {
//                 user?.user.user.instagram && (
//                       <a
//                         href={user?.user.user.instagram}
//                         target="_blank"
//                         rel="noopener"
//                         className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"
//                       >
...
//                       placeholder="Coloque seu facebook"
//                     />
//                   </div>
...
//                       placeholder="Coloque seu instagram"
//                     />
//                   </div>
...
//                       placeholder="Coloque seu Youtube"
//                     />
//                   </div>
